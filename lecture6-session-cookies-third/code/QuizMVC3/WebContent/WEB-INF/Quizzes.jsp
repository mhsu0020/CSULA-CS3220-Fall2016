...
<body>
	...
	<%--Access session attributes using the sessionScope object --%>
	<h1>Welcome ${sessionScope.username}! <a href="Logout">Logout</a></h1>
	<c:choose>
	    <c:when test="${sessionScope.isAdmin}">
	       <h3><strong>You are an admin</strong></h3>   
	    </c:when>
	    <c:otherwise>
	...
	<tr><th>Question Text</th><th>Difficulty Rating</th><th>Actions</th></tr>
	...
	<c:forEach items="${quizzes}" var="quiz" varStatus="loop">
      <tr>
		...
		<td>
		<a href="ViewQuiz?id=${quiz.id}">View</a>
			<%-- only show edit and delete if user is admin --%>
			<c:if test="${sessionScope.isAdmin}">
				|<a href="EditQuiz?id=${quiz.id}">Edit</a>|<a href="DeleteQuiz?id=${quiz.id}">Delete</a>
			</c:if>
		</td>
      </tr>
	</c:forEach>
